...
- Cet utilisateur a t-il le droit de faire tel action ?
- Exemple : Le User avec le `Role Admin` peut t-il supprimer une donnée ?

Pour le moment nous avons quelque règle simple basé sur le `Role`

- Si `admin` on peut tout faire sinon (`user`) accès refusé
...
Mais dans une application complexe ou peut avoir besoin de gerer encore plus financement
...
Et il devient vite compliqué d’implémenter toutes ces règles et de les maintenir. Des services tiers nous permettent de gérer cela ou des librairies
...
**🐶** Dans cette exercice tu vas devoir utiliser `accesscontrol` pour gérer le RBAC.

Etape 1 : configuration

- `rbac-config.ts`
- Définie la configuration pour les rôles `admin`, `user` et `public`
...
Nous avons créé un service générique “`authorization`” qui va gérer le RBAC. L’idée de de pouvoir appeler une fonction `permissionAcces` qui appellera Access control del a manière suivante
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20Mastery&entry.1430994900=8.SAAS%20Entrepreneur&entry.533578441=11%20RBAC).
