...
Comme c’est 80% identique pour la partie tableau, les exercices se concentrons sur les différences , le reste sera fourni.

## Exercice

Un différence majeure par rapport à la feature `Finance` est que nous avons en plus des années, une agrégation par semaine. Nous voulons pouvoir tracker ce qu’un utilisateur a fait sur une semaine du lundi au dimanche la semaine X. Exemple

- Lundi : Poids : 80 Kilos | Sport : 2 heures | Calories : 2000k
- Mardi : Poids : 81 Kilos | Sport : 1 heures | Calories : 2200k
- etc …

Les 2 grandes fonctions majeures ç adapter / créer sont

- `getHealthsWithPaginationByWeekDao` : Les `heaths` par année et semaine pour un `user`
- `getWeeksHealthsByYearDao`: Les `weeks` par `année` ou des données sont disponibles. Les semaines disponibles pourront etre inserer dans un seconde `<Select>` pour avoir par exemple les données du
  - 2025 : Semaine du ‘28-07 au 03-08’

**🐶 Implémente ces 2 fonctions de le `repository Health`**

💡 Astuce pout `getHealthsWithPaginationByWeekDao` : `Postgrees` permet de filtrer par années comme cela
...
💡 Astuce pout `getWeeksHealthsByYearDao` :

- Filtrer par `année` par `user`
- Concaténer avec [CONCAT](https://neon.tech/postgresql/postgresql-string-functions/postgresql-concat-function#summary)
- Avoir le lundi avoir [DATE_TRUNC(’week’)](https://neon.tech/postgresql/postgresql-date-functions/postgresql-date_trunc#summary)
- Avoir le vendredi avec [`DATE(... + INTERVAL '6 day')`](https://neon.tech/postgresql/postgresql-tutorial/postgresql-interval)
...
Pour filtrer par semaine, la première chose a faire et d’ajouter un `queryParam` : `healthWeek` à la route `/heath`, par exemple :

- `/health?healthWeek=28-07+au+03-08`

Dans un second temps il va falloir convertir cette chaine ‘`28-07+au+03-08`’ en numéro de semaine dans l’année, pour cela tu as a ta disposition `getWeekFromLabel`
...
- 🐶 Dans un premier temps , adapte la route (`app/(app)/health/page`) pour avoir les bons params et faire les bon appels aux services
- 🐶 Dans un second temps, occuper toi du `Select Year Week` (`health-year-select.tsx`)
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20Mastery&entry.1430994900=8.SAAS%20Entrepreneur&entry.533578441=06-feature-health-crud).
