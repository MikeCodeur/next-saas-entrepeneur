...
Pour utiliser les tests dans notre projet, nous avons ajouté les dépendances suivantes :
...
- Nous avons créé un fichier de configuration `vitest`
...
Dans cet exercice, tu vas devoir tester une des parties les plus importantes de l’application, la `couche service`.

En appelant des fonctions du service, cela va utiliser la validation, l’autorisation et les repositories.

- Nous allons `mocker` les repositories (qui peuvent être testés séparément). Exemple :
...
- Nous allons rédiger des jeux de tests pour s’assurer que les cas fonctionnels liés au `RBAC` sont respectés.

**🐶** Dans un premier temps, commence par les tests du service user (`CRUD`) avec différents rôles (`USER` `ADMIN` `PUBLIC`)

- Nous avons à notre disposition `setupUserAuthExtented`, qui permet de simuler l’utilisateur connecté. Cela sera utile pour chaque test.
...
- Test qu'un administrateur peut voir les données de santé de tous les utilisateurs
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20Mastery&entry.1430994900=9.SAAS%20Entrepreneur&entry.533578441=15%20tests-rbac).
