...
### 💡 Voilà une spécification fonctionnelle
...
**👨‍✈️** Hugo, Le chef de projet, te donne une spécification fonctionnelle (formalisme `agil scrum`)
...
Dans cet exercice, tu vas commencer par afficher les données `finance` dans un tableau.

La première partie va consister à modéliser `Finance` avec `Drizzle` et créer un `repository` et un `service`.

Pour Rappel :

- `Model` : Définition des tables, colonnes, relations
- `Repository` : Accès aux données
- `Service` : Validation des données, autorisation
...
- `createFinanceByUidDao` : Création d’une finance pour un user spécifique
...
- `getYearsFinancesByUidDao` : Toutes les années finance d’un `userId`
- `getFinancesWithPaginationByYearDao` : Toutes les finances d’un `userId` , filtrées par année, avec une pagination triée par date
...
🐶 Step 4 : Route Finance : Appelle toutes les fonctions du service dans la route `/finance` (avec les bons paramètres), dans un premier temps, ne te soucie pas de l’affichage en tableau nous le verrons dans l’exercice suivant :
...
📑 Le lien vers la doc [https://tanstack.com/table/v8/docs/guide/column-defs](https://tanstack.com/table/v8/docs/guide/column-defs)
...
- 🐶 Crée la `pagination` dans `data-table-pagination.tsx`. Le composant est repris de `chadcn` mais il faut choisir le comportement d’implémentation. Nous allons ici utiliser le `router` et changer les `queryParams` de la route `finance`. nous l’avons testé précédemment et nos `queryParam` sont censés retourner les bonnes données
...
Remplir le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20Mastery&entry.1430994900=9.SAAS%20Entrepreneur&entry.533578441=05%20Feature%20finance-crud-table).
