...
export const permissionAcces = (
  ...
  //mike : why admin in public role ?
  // if (user?.role.includes("admin") || user === undefined) {
  //   return ac.can(user?.role ?? "public")[`${action}Any`](ressourceType)
  // }
  // if (ressourceUid && user.id === ressourceUid) {
  //   return ac.can(user.role)[`${action}Own`](ressourceType)
  // }
