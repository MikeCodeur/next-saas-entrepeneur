...
vi.mock("@/data/repositories/finance-repository", () => ({
  ...
  deleteFinanceByUidDao: vi.fn(),
  getFinanceByIdDao: vi.fn(),
  updateFinanceByUidDao: vi.fn(),
}))
...
describe("[deleteFinanceById] Quand la fonction est appelée", () => {
  ...
  it("[USER] devrait appeler 'deleteFinanceDao' si l'utilisateur est l'auteur", async () => {
    await deleteFinanceByid(financeId)
    expect(financeRepository.deleteFinanceByUidDao).toHaveBeenCalledWith(
      financeId,
      financeUserId
    )
  })
...
describe("[updateFinance] Quand la fonction est appelée", () => {
  ...
  it("[USER] devrait appelé `updateFinanceByUidDao` si l'utilisateur est l'auteur", async () => {
    ...
    expect(financeRepository.updateFinanceByUidDao).toHaveBeenCalledOnce()
    expect(financeRepository.updateFinanceByUidDao).toHaveBeenCalledWith(
      financeParams,
      financeUserId
    )
  })
