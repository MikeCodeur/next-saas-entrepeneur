...
export const getFinancesByYearDal = cache(
  async (year: string, uid: string, page: number, pageSize: number) => {
    try {
      ...
      //const dtoByAc = await financesDTOAccessControl(finances.data)
      const dto = await financesDTO(finances.data)
      console.log('dto', dto)
      return {
...
// export async function financesDTOAccessControl(
//   finances: Finance[]
// ): Promise<FinanceDTO[] | never[]> {
//   return await filterFinancesAttributes(finances)
// }
